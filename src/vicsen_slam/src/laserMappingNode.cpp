    ...
    while(1){
        if(!odometryBuf.empty() && !pointCloudBuf.empty()){
            ...
            if(displacement>0.3 || angular_change>20){
                ...
                pcl::PointCloud<pcl::PointXYZ>::Ptr pc_map = laserMapping.getMap();
...
*/
    ...
    //nh.getParam("/horizontal_angle", horizontal_angle);
    ...
    //lidar_param.setHorizontalAngle(horizontal_angle);
    ...
    ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/points_filtered", 100, lidarHandler);
