...
void LaserMappingClass::init(double map_resolution){
	...
	for(int i=0;i<LASER_CELL_RANGE_HORIZONTAL*2+1;i++){
		std::vector<std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>> map_height_temp;
		for(int j=0;j<LASER_CELL_RANGE_HORIZONTAL*2+1;j++){
			std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> map_depth_temp;
			for(int k=0;k<LASER_CELL_RANGE_VERTICAL*2+1;k++){
				pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud_temp(new pcl::PointCloud<pcl::PointXYZ>);
...
void LaserMappingClass::addWidthCellNegative(void){
	...
	for(int j=0; j < map_height;j++){
		...
		for(int k=0;k< map_depth;k++){
			pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud_temp;
			map_depth_temp.push_back(point_cloud_temp);
		}
		map_height_temp.push_back(map_depth_temp);
	}
...
void LaserMappingClass::updateCurrentPointsToMap(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pc_in, const Eigen::Isometry3d& pose_current){	
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr down_sized_pc(new pcl::PointCloud<pcl::PointXYZ>());
	...
	{
		pcl::PointXYZ point_temp = transformed_pc->points[i];
...
pcl::PointCloud<pcl::PointXYZ>::Ptr LaserMappingClass::getMap(void){
	pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudMap = pcl::PointCloud<pcl::PointXYZ>::Ptr(new  pcl::PointCloud<pcl::PointXYZ>());
	for (int i = 0; i < map_width; i++){
		for (int j = 0; j < map_height; j++){
			for (int k = 0; k < map_depth; k++){
				if(map[i][j][k]!=NULL){
