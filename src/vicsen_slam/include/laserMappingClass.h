...
class LaserMappingClass 
{
    ...
    public:
		...
		void updateCurrentPointsToMap(const pcl::PointCloud<pcl::PointXYZ>::Ptr& pc_in, const Eigen::Isometry3d& pose_current);
		pcl::PointCloud<pcl::PointXYZ>::Ptr getMap(void);
	...
	private:
		...
		int map_height;
		int map_depth;
		std::vector<std::vector<std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr>>> map;
